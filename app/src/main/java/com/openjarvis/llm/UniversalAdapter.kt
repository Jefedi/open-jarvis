...
class UniversalAdapter(private val context: Context) {
    ...
    private val prefs = EncryptedSharedPreferences.create(context, "jarvis_encrypted_prefs", masterKey,
    ...
    fun getActiveProvider(): LLMProvider {
        ...
        return when (name) {
            ...
            "Local (LlamaCpp)" -> {
                val manager = getModelManager(context)
                val tier = manager.getDownloadedTier() ?: ModelManager.ModelTier.MINIMUM
                LlamaProvider(manager.getModelPath(tier), tier.displayName)
            }
    ...
    fun saveSettings(name: String, baseUrl: String, apiKey: String, model: String) {
        prefs.edit().putString("provider_name", name).putString("provider_base_url", baseUrl)
            .putString("provider_api_key", apiKey).putString("provider_model", model).apply()
    }
    fun getProviderName(): String = prefs.getString("provider_name", "Groq") ?: "Groq"

    companion object {
        @Volatile private var modelManager: ModelManager? = null
        val AVAILABLE_PROVIDERS = listOf("Groq", "Google Gemini", "OpenRouter", "Anthropic Claude",
            "OpenAI", "Ollama (Local)", "Local (LlamaCpp)", "Custom")
        fun getModelManager(context: Context): ModelManager = modelManager ?: synchronized(this) {
            modelManager ?: ModelManager(context.applicationContext).also { modelManager = it }
        }
        fun isLocalModelLoaded(): Boolean = modelManager?.state?.value is ModelManager.ModelState.Loaded
